  ...
  if (tweet.in_reply_to_screen_name && tweet.entities.media) {
    ...
    function downloadFile(url, filename) {
      ...
      function downloaded(err, res, body) {
        ...
        request(url)
          .pipe(fs.createWriteStream("PicPixBot/" + filename))
          .on("close", filed);
              ...
              if (mediaIdStr && name) {
                var params = {
                  status:
                    ...
                    name +
                    " " +
                    phrase[getRandomInt(0, phrase.length)] +
                    " #PicPixBot",
...
function intervalTweet(err, data, response) {
  ...
  for (var i = 0; i < result.length; i++) {
    ...
    if (
      typeof result[i].entities.media !== "undefined" &&
      result[i].entities.media[0].type == "photo"
    ) {
        ...
        function processing2(error, stdout, stderr) {
          ...
          function uploaded2(err, data, response) {
            ...
            if (mediaIdStr) {
              var params = {
                status:
                  phrase2[getRandomInt(0, phrase2.length)] + " #PicPixBot",
                media_ids: [mediaIdStr]
              };
            } else {
              var params = {
                status:
                  errorphrase[getRandomInt(0, errorphrase.length)] +
                  "\n(Whoops. Something went wrong. Please try again in a few seconds!) #PicPixBot",
                in_reply_to_status_id: id
              };
            }
