...
void setup() {
  ...
  if (strparameters.length < 4) {
    ...
    for (int i = 1; i < 4; i++) {
      parameters[i] = int(strparameters[i]);
    }
  }
  if (int(parameters[1]) < 0 || int(parameters[1]) > 9 || int(parameters[2]) < 2 || int(parameters[2]) > 256 || int(parameters[3]) < 2 || int(parameters[3]) > 124) {
    problem = true;
  }
  if (problem != true) {
    ...
    outimg = createGraphics(inimg.width, inimg.height);
    pixeltype = int(random(1, 9));
    samplesize = int(random(2, 50));
    clusternum = int(random(10, 100));
  }
}
...
void palette() {
  ...
  while (pcolors < samplesize) {
    ...
    maxed = int(clusterposition[mostfrequent][3]);
...
void scircle() {
  ...
  for (int y = 0; y < inimg.height + samplesize; y = y + samplesize) {
    ...
    for (int x = 0; x < inimg.width + samplesize; x = x + samplesize) {
      ...
      if (odd) {
        outimg.ellipse(x, y, samplesize * 1.1, samplesize);
        odd = false;
      } else {
        outimg.ellipse(x, y+samplesize/2, samplesize * 1.1, samplesize);
        odd = true;
      }
    }
  }
...
void cluster() {
  ...
  boolean again = false;
  for (int i = 0; i < clusternum; i++) {
    position[0] = int(random(0, 100));
    position[1] = int(random(0, 100));
    position[2] = int(random(0, 100));
    position[3] = 1;
    clusterposition[i] = float(position);
    again = false;
